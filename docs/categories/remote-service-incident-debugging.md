# Category: remote-service-incident-debugging

> Deep-dive for the **1 skill** in the `remote-service-incident-debugging` category. Each section embeds the full `SKILL.md` body so this doc is self-contained.

**Skills in this category:**

- [`remote-service-incident-debugging`](../skills/remote-service-incident-debugging/remote-service-incident-debugging.md) - Debug a stuck or broken production service running on a remote host (VPS, container, server) over SSH, with no source tr
...
- **Author:** Hermes Agent
- **License:** MIT
- **On-disk path:** `C:/Users/soren/AppData/Local/hermes/skills/remote-service-incident-debugging/SKILL.md`

**Description:**

> Debug a stuck or broken production service running on a remote host (VPS, container, server) over SSH, with no source tree at the local cwd. Use when a user reports an app is "stuck on a loading screen", "frozen on splash", "won't connect", returns 502, or has a dead listener — and the only way in is SSH.
